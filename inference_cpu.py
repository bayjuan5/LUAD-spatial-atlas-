...
#     """Loads state_dict, stripping the 'module.' prefix if necessary."""
#     try:
#         # 1. Load the raw state_dict. Using weights_only=False to bypass
#         #    the NumPy serialization error you encountered previously.
#         #    Use map_location to ensure it loads onto the correct device (cpu).
#         #    Alternatively, if using the safer fix:
#         #    with safe_globals([numpy.core.multiarray.scalar]):
#         #        state_dict = torch.load(model_path, map_location=device, weights_only=True)
#
#         state_dict = torch.load(model_path, map_location=device, weights_only=False)
#
#         # Check if the state dict is wrapped in 'module.' (from DataParallel)
#         is_dataparallel_wrapped = any(k.startswith('module.') for k in state_dict.keys())
#
#         if is_dataparallel_wrapped:
#             # 2. Fix: Create a new state dict by stripping the 'module.' prefix
#             new_state_dict = {}
...
#         else:
#             # Load directly if no 'module.' prefix is found
#             model.load_state_dict(state_dict)
#
#         print(f"✅ Loaded model weights from {model_path}")
#
#     except Exception as e:
#         # This catches errors if the model architecture itself is wrong, or the file is corrupted.
...
def load_model_weights(model, model_path, device):
    """Loads a PyTorch checkpoint, extracts model weights, and strips the 'module.' prefix."""
    try:
        # 1. Load the entire checkpoint dictionary (using weights_only=False as you did before)
        checkpoint = torch.load(model_path, map_location=device, weights_only=False)

        # 2. EXTRACT: Find the actual model weights.
        # Your error shows the weights are nested under 'model_state_dict'.
        if 'model_state_dict' in checkpoint:
            state_dict = checkpoint['model_state_dict']
        else:
            # Fallback for raw state dict files, though unlikely now
            state_dict = checkpoint

        # 3. STRIP: Remove the 'module.' prefix from the keys.
        new_state_dict = {}
        for k, v in state_dict.items():
            # Check if the key starts with 'module.' and strip it
            name = k.replace('module.', '')
            new_state_dict[name] = v

        # 4. LOAD: Load the cleaned state dict into the model
        model.load_state_dict(new_state_dict)
